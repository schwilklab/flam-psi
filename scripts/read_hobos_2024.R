...
burn_trials_2024 <- burn_trials_2024 %>%
  filter(! sample_id %in% c("DK07", "ST21", "ST28")) %>%
...
flam_left <- concat_hobo_files(list.files("./data/2024/burn_hobos",
...
flam_mid <- concat_hobo_files(list.files("./data/2024/burn_hobos",
                                         full.names = TRUE, recursive = TRUE,
...
flam_right <- concat_hobo_files(list.files("./data/2024/burn_hobos",
                                           full.names = TRUE, recursive = TRUE,
...
get_trial_label <- function(time) {
  matches <- time %within% burn_trials_2024$interval
  if(! any(matches)) return(NA)
  ## TODO: rewrite below to be clearer that there should be just one match.
  return(burn_trials_2024$sample_id[which.max(matches)])
}
...
###############################################################################
hobos_wider_2024 <- hobo_temp_sum %>%
  group_by(sample_id) %>%
  summarise(dur_100 = mean(dur_100),
            peak_temp = max(peak_temp),
            degsec_100 = max(degsec_100))

dim(hobos_wider_2024)


## Save RDS data. TODO
saveRDS(hobos_wider_2024, file.path(DATA_CACHE_DIR, "hobos_wider_2024.RDS"))
